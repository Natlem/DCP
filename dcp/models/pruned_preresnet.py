...
__all__ = ['PrunedPreResNet', 'PrunedPreBasicBlock']
...
class PrunedPreBasicBlock(nn.Module):
    """
    base module for PrunedPreResNet on small data sets
    """

    def __init__(self, in_plane, out_plane, pruning_rate, stride=1, downsample=None, block_type="both_preact"):
        """
        ...
        self.conv1 = conv3x3(in_plane, self.pruned_channel_plane, stride)
        self.bn2 = nn.BatchNorm2d(self.pruned_channel_plane)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(self.pruned_channel_plane, out_plane)
        self.block_index = 0
...
class PrunedPreResNet(nn.Module):
    """
    ...
    def __init__(self, depth, pruning_rate, wide_factor=1, num_classes=10, is_mnist=False):
        ...
        """
        super(PrunedPreResNet, self).__init__()
    ...
    def _make_layer(self, block, out_plane, n_blocks, pruning_rate, stride=1):
        """
        ...
        layers = []
        layers.append(block(self.in_plane, out_plane, pruning_rate, stride,
                            downsample, block_type="half_preact"))
        self.in_plane = out_plane
        for i in range(1, int(n_blocks)):
            layers.append(block(self.in_plane, out_plane, pruning_rate))
